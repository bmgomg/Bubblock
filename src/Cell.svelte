<script>
	...
	import Star from '$lib/images/Star.webp';
...
{:else if isSolved() && !isAnimated()}
	<div id="cell-{cell.id}" class="cell" {style} in:fade>
		<img class="star" src={Star} alt="" width="70%" />
	</div>
{:else if !ss.practice && !ss.timer && !isAnimated()}
	<div id="cell-{cell.id}" class="cell instant" {style} in:fade>
		<div class="cross">×</div>
	</div>
{/if}
...
<style>
	...
	.star {
		grid-area: 1/1;
		place-self: center;
		filter: drop-shadow(0 0 5px #000000c0);
	}
